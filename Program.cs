        ...
        private static void initialize()
        {
            ...
            for (int i = 0; i < wells.Length; i++)
            {
                ...
                if (i == 3)
                {
                    ...
                    well.type = Well.Type.Specified_Flow_Rate;
            ...
            TypeDefinitions.Compressibility compressibility = TypeDefinitions.Compressibility.Incompressible;
            ...
            for (int counter = 0; counter < grid.Length; counter++)
            {
                ...
                else if (compressibility == TypeDefinitions.Compressibility.Slightly_Compressible)
                {
                    ...
                    block.Cf = compressibility_rock; block.Cw = compressibility_fluid;
                ...
                block.boundary_q = boundary_flow_rate[counter];
                if (boundary_pressure_x[counter] != 0)
                {
                    ...
                    block.boundary_p = boundary_transmissibility * boundary_pressure_x[counter];
                    block.boundary_T = boundary_transmissibility;
                }
                else if (boundary_pressure_y[counter] != 0)
                {
                    ...
                    block.boundary_p = boundary_transmissibility * boundary_pressure_y[counter];
                    block.boundary_T = boundary_transmissibility;
                }

                //boundary pressure gradient is positive for depletion and negative for encroachment
                boundary_transmissibility = Transmissibility.getBoundaryTransmissibility(block, grid_type, direction_x, phase);
                block.boundary_gradient += boundary_pressure_gradient_x[counter] * block.delta_x / 2 * boundary_transmissibility;

                boundary_transmissibility = Transmissibility.getBoundaryTransmissibility(block, grid_type, direction_y, phase);
                block.boundary_gradient += boundary_pressure_gradient_y[counter] * block.delta_y / 2 * boundary_transmissibility;
                ...
                if (wells[counter] != null)
                {
                    ...
                    if (well.type == Well.Type.Specified_Flow_Rate)
                    {
            ...
            {
                Solver.incompressible(grid);
            }
            else if (compressibility == TypeDefinitions.Compressibility.Slightly_Compressible)
            {
                Solver.slightly_compressible(grid, delta_t, time_max);
            }
            else
            {
                //Solver.compressible(grid, delta_t, time_max, convergence_pressure, pvt);
            }
            #endregion
        }      
    }
